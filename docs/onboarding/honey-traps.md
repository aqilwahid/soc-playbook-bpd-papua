# Bank Papua SOC Honey Trap (Pilot)

## Overview

!!! note

    This Honey Trap initiative is a pilot program for the Bank Papua SOC, as such the number of participants for the pilot is limited.
    If interested in participating, please raise a request via the [Bank Papua SOC IRP](https://irp.dpc.wa.gov.au/).

The Bank Papua SOC is looking to expand the monitoring capability of Western Australian Government entities to better detect for advanced persistent threats (APT) on ICT infrastructure. The Bank Papua SOC will assist entities with the deployment of honey pots and honey tokens in infrastructure locations that are commonly traversed by Threat Actors prior to a cyber incident.
...
??? note "Pilot Participants Prerequisite"
    ...
    - Must have access to the [Bank Papua SOC IRP](https://irp.dpc.wa.gov.au/) to raise Honey Traps request tickets.
    - Must be able nominate a individual from your organisation to work closely with the Bank Papua SOC Engineering Team during the pilot.

    The onboarding to the Honey Trap Platform is handled entirely by the Bank Papua SOC Engineers and can be facilitated upon request via the [Bank Papua SOC IRP](https://irp.dpc.wa.gov.au/).

    The Bank Papua SOC is willing to work with entities to adapt the platform as fit for purpose for their environment but may not be able to take full advantage of the service offering.

## Onboarding Process

The Bank Papua SOC has developed guidance to assist WA Government entities to integrate the Honey Trap platform into their exisiting Sentinel Workspaces. This can be found [here](https://github.com/wagov/wasoc-honeytraps).

## Honey Trap Guidance

The Bank Papua SOC recommends that entities explore the following locations within their infrastructure to deploy Honey Traps effectively for each applicable honey technology.
