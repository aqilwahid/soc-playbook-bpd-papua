# Bank Papua SOC Onboarding Procedure

## 1. Overview

!!! note

    Prosedur ini berfokus pada pembentukan konektivitas dengan Bank Papua SOC, silakan merujuk pada [panduan kami tentang konfigurasi sentinel dan defender](onboarding/sentinel-guidance.md) untuk cara mengimplementasikan perangkat terkait dengan cepat.
...
As part of onboarding, the Bank Papua SOC will send the customer a prefilled [Azure Lighthouse ARM Deployment](https://docs.microsoft.com/en-us/azure/lighthouse/how-to/onboard-customer#create-your-template-manually) that can be installed as an **Service provider offer** to initiate an [Azure Lighthouse](https://docs.microsoft.com/en-us/azure/lighthouse/overview) connection between the customer Azure Subscription and the Bank Papua SOC Tenant. Once completed the WSOC can delegate relevant permissions to analysts and automation processes via privileged groups in the Bank Papua SOC tenant, allowing it to service the customers Azure subscription. This process needs to be undertaken for each subscription the customer would like to delegate access to.

### 1.2. Microsoft XDR tenant access

![Defender XDR](images/xdr-overview.png)

As part of onboarding, the Bank Papua SOC will send the customer a [Microsoft Entra ID Security group Object ID](https://learn.microsoft.com/en-us/entra/fundamentals/how-to-manage-groups#edit-group-settings) to allow delegated access to a specific group of users within the tenancy. This Security Group ID is applicable for step (TBD).
...
- List of agency Azure AD Identities (as emails) that will require access to the Bank Papua SOC Incident Reporting Portal for collaboration on cyber security incidents.
...
Browse for the template provided, and click **Upload**. This can be customised to removed unused groups if desired for the customers Tier - please inform the Bank Papua SOC of any changes prior to deployment to allow documentation to be updated.
...
??? note "Customer Prerequisite"
    ...
    - Must have signed an exisiting MOU (T0,T1,T2) with the Bank Papua SOC
    - Customers Workspace must be located in region **Australia EAST**
    - Must have been already onboarded to the Bank Papua SOC via [Azure Lighthouse](https://soc.cyber.wa.gov.au//onboarding/#23-azure-subscription-access-delegation)
...
The onboarding to the Dedicated Cluster is handled entirely by the Bank Papua SOC Engineers and can be facilitated upon request.

## 4. Microsoft XDR Onboarding

### 4.1. Microsoft Entra ID B2B Synchornisation

The Bank Papua SOC leverages the [Microsoft Entra ID cross-tenant synchronisation services](https://learn.microsoft.com/en-us/entra/identity/multi-tenant-organizations/cross-tenant-synchronization-configure) to replicates Bank Papua SOC analysts indentities from the source tenancy to the entities tenancy. This allows Bank Papua SOC analysts to authenticate to entities XDR environment seamlessly.

### 4.1.1 Enable User Synchronisation with the Bank Papua SOC
...
1. Add the **External Microsoft Entra Tenant ID** provived by the Bank Papua SOC
...
1. Add the *Group Object ID* provided by the Bank Papua SOC and click **Submit**
...
1. In **Configure Rules**, amend a rule where user field Property **companyName** > Operator **Equals** > Value **Bank Papua SOC**
...
1. In Set up the basics fill the following fields
    - Role Name: Bank Papua SOC Team
    - Description: Bank Papua SOC Team access to XDR and Microsoft Security Stack
...
Once the template phase has completed, customers can confirm the onboarding process has finalised by navigating to the [Azure Lighthouse - Service Providers](https://portal.azure.com/#view/Microsoft_Azure_CustomerHub/ServiceProvidersBladeV2/~/providers) page and confirming you can see the **Bank Papua SOC - Security Insights** service offer.

![service Offer](images/service-offer.png)

### 5.2 XDR Onboarding

Once XDR onboarding procedure has been completed by the entity, the Bank Papua SOC will commence some addtional proceesses to finalise the onboarding process.
The Bank Papua SOC will inform the entity once the onboarding has been completed.

## 6. Bank Papua SOC Offboarding / Re-onboarding Procedure

### 6.1. Sentinel Offboarding / Re-onboarding

If for the purpose of offboarding the Bank Papua SOC or to re-onboard onto the Bank Papua SOC, then the customer has the ability to self manage this process via the **Azure Portal**.

#### Azure Lighthouse Service Provider

Navigating to the [Azure Lighthouse - Service Providers](https://portal.azure.com/#view/Microsoft_Azure_CustomerHub/ServiceProvidersBladeV2/~/providers) page. Select the **Bank Papua SOC - Security Insights** service offer. Click **Delete**.

![service offer delete](images/service-provider-delete.png)

### 6.2. XDR Offboarding

If for the purpose of offboarding the Bank Papua SOC off the XDR platform, then the customer has the ability to self manage this process via the **Azure Portal**.
...
1. On the **Organization settings** tab, navigate to the organisation associated to the Bank Papua SOC.
