...
!!! note "Under Review"
    ...
    The Bank Papua SOC's older detection and automation packs are currently being converted into an [external content repository](https://learn.microsoft.com/en-us/azure/sentinel/ci-cd?tabs=github) to enable better change management with git.

To improve detection coverage within Microsoft Sentinel, please follow the Bank Papua SOC [telemetry re-ingestion](../onboarding/sentinel-guidance.md#3-third-party-solutions-telemetry-re-ingestion) and [incident synchronisation](../onboarding/sentinel-guidance.md#4-third-party-integrations-incident-synchronisation-only) guidance.
